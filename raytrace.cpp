...
private:
	...
	RGB shading(Object *obj, Ray ray, Point cur, Point norm, int depth, double weaken) {
		...
		for (auto &lig : lights) {
			...
			if (!is_blocked) {
				double dist_coef = min(1., 1 / (0.0005 * lig.o.dist2(cur)));
				if (!inside && cos_theta > 0) {
					color += dist_coef * lig.color * obj->mat.diff * cos_theta; // diffuse
					color += dist_coef * lig.color * obj->mat.spec * pow(max(1., rRay.dir.dot(sRay.dir)), obj->mat.coef_n); // specular reflection
				}
				if (cos_theta < 0) {
...
public:
	RayTrace() {
		MAX_DEPTH = 12;
		int SCALE = 1;
		camera = Camera(4800 / SCALE, 3600 / SCALE, Point(60, 80, 100), Point(0, -5, 4), SCALE * .04);
	}
